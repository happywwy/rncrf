...
def forward_prop(params, tree, d, c, labels=True):
    ...
    while to_do:
        ...
        if len(curr.kids) == 0:
            ...
            #add another vector for forward computation
            curr.up = tanh(Wv_up.dot(curr.vec) + b_up)
            ...
            # otherwise, compute p at node
                ...
                for ind, rel in curr.kids:
                    ...
                    try:
                        ...
                        #compute sum from up vector
                        kid_sum += rel_dict[rel].dot(curr_kid.up)
...
def backprop(params, tree, d, c, len_voc, grads, mixed = False):
    ...
    (rel_dict, Wv, Wv_up, Wc, b, b_up, b_c) = params
    ...
    while to_do:
        ...
        #add delta for forward vector
        curr_der_up = der_tanh(node.up)
        node.delta_up = np.multiply(delta_down, curr_der_up)
        ...
        if len(node.kids) > 0:
            ...
            for ind, rel in node.kids:
                ...
                grads[0][rel] += node.delta_full.dot(curr_kid.up.T)
            ...
            #2 represents Wv_up
            grads[2] += node.delta_up.dot(node.vec.T)
            ...
            if mixed:
                ...
                grads[7][50:, node.ind] += Wv.T.dot(node.delta_full).ravel()[50:] + Wv_up.T.dot(node.delta_up).ravel()[50:]
            else:
                #grads[5][:, node.ind] += Wv.T.dot(node.delta_full).ravel()
                grads[7][:, node.ind] += Wv.T.dot(node.delta_full).ravel() + Wv_up.T.dot(node.delta_up).ravel()
