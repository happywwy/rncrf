...
def backprop(params, tree, d, c, len_voc, grads, mixed = False):
    ...
    (rel_dict, Wv, Wv_up, b, b_up) = params
    ...
    while to_do:
        ...
        if len(node.kids) > 0:
            ...
            #4 represents b_up
            grads[4] += node.delta_up
            #grads[4] += delta_bc
            if mixed:
                #grads[5][50:, node.ind] += Wv.T.dot(node.delta_full).ravel()[50:]
                #grads[3][50:, node.ind] += Wv.T.dot(node.delta_full).ravel()[50:]
                grads[5][50:, node.ind] += Wv.T.dot(node.delta_full).ravel()[50:] + Wv_up.T.dot(node.delta_up).ravel()[50:]
            else:
                #grads[3][:, node.ind] += Wv.T.dot(node.delta_full).ravel()
                grads[5][:, node.ind] += Wv.T.dot(node.delta_full).ravel() + Wv_up.T.dot(node.delta_up).ravel()
