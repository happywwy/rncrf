    ...
    while to_do:
        ...
        if len(node.kids) > 0:
            ...
            for ind, rel in node.kids:
                ...
                #no delta_down, but update kid's word vector
                to_do.append( (curr_kid, zeros( (d, 1) ) ) )
                if mixed:
                    grads[5][50:, curr_kid.ind] += rel_dict[rel].T.dot(node.delta_full).ravel()[50:]
                else:
                    grads[5][:, curr_kid.ind] += rel_dict[rel].T.dot(node.delta_full).ravel()
